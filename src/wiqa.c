...
/* KryptoMagick WIQA Cipher A */

unsigned char *wiqa_crypt(unsigned char *data, unsigned char *key, unsigned char *nonce, long datalen, int keylen, int noncelen) {
    int diff = 256 - keylen;
    int k[256] = {0};
    int j = 0;
    int i = 0;
...
unsigned char * wiqa_random (unsigned char *buf, int num_bytes) {
    int keylen = 32;
    int noncelen = 16;
    ...
    fclose(randfile);
    wiqa_crypt(buf, key, nonce, num_bytes, keylen, noncelen);
}
